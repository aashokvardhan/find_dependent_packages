...
def install_package(package_name):
    pip_install = subprocess.Popen(["pip3", "install", "--extra-index-url=https://pypi.python.org/simple/", package_name],
...
if __name__ == '__main__':
    ...
    if not found:
        print("specified package not found")
    else:
    ...
    if len(depend_package)==0:
        f.write(str([args.package,args.version,[]]))

    for package in depend_package:
        for version in depend_package[package]:
             print([package,version,depend_package[package][version]])
             f.write(str([package,version,depend_package[package][version]]))
             f.write('\n')
